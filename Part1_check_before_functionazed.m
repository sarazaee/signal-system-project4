%----------------- part 1, converting the massage to binary form -------------
message = 'signal';
...
%------------- part 2, finding the best part of image to hide the message ----

myImage = imread('testPic.jpg');
...
for n = 1:block_size:(rows-block_size+1) 
    for m = 1:block_size:(cols-block_size+1)
        ...
        block = double(myGrayImage(n:n+block_size-1, m:m+block_size-1));
        blockVar = var(block(:));
...
figure;
subplot(1,2,1); imshow(myGrayImage); title('Original gray Image');
